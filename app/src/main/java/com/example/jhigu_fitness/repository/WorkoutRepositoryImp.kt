...
class WorkoutRepositoryImp : WorkoutRepository {
    ...
    override fun addWorkout(productModel: WorkoutModel, callback: (Boolean, String) -> Unit) {
    ...
    override fun getWorkoutById(
        ...
        ref.child(productId).addValueEventListener(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                if (snapshot.exists()) {
                    var model = snapshot.getValue(WorkoutModel::class.java)
    ...
    override fun getAllWorkout(callback: (List<WorkoutModel>?, Boolean, String) -> Unit) {
        ref.addValueEventListener(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                if (snapshot.exists()) {
                    var products = mutableListOf<WorkoutModel>()
                    for (eachData in snapshot.children) {
                        var model = eachData.getValue(WorkoutModel::class.java)
                        if (model != null) {
                            products.add(model)
                        }
                    }
    ...
    override fun getCurrentWorkout(): FirebaseUser? {
    ...
    override fun getWorkoutByCategory(
        ...
        ref.child(userID).addValueEventListener(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                if (snapshot.exists()) {
                    val userModel = snapshot.getValue(WorkoutModel::class.java)
                    Log.d("userId", userModel?.productName.toString())
